...
export const HUD: React.FC<HUDProps> = ({
    ...
    // const isMobile = useIsMobile(); // Use prop instead
    ...
    return (
        <div className={`compact-hud${isMobile ? ' mobile-hud' : ''}`}>
            <div className={`compact-instrument-panel${isMobile ? ' mobile-panel' : ''}`}>
                ...
                {/* 3. Audio (Derecha) - Visible también en móvil */}
                <div className={`compact-mute-toggle ${isMuted ? 'muted' : 'unmuted'}${isMobile ? ' mobile-mute' : ''}`} onClick={onToggleMute}>
                    <div className={`compact-instrument-label${isMobile ? ' mobile-label' : ''}`}>AUDIO</div>
                    <div className={`compact-mute-icon${isMobile ? ' mobile-mute-icon' : ''}`}>
                        {isMuted ? (
